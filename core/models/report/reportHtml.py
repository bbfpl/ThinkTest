        ...
        <div class="col-md-12">
            ...
            <div class="col-md-8">
                ...
                <script type="text/javascript">
                    ...
                    color: ['#3c763d', '#FE9A2E', '#FF4000', '#0099CC'],
...
projectName = '测试'
EnvName = '测试2'
html_template = html_template.replace('{**}', projectName + EnvName)
    ...
    <div class="col-md-12">
        <div class="tabbable" id="tabs-957640">
            <ul class="nav nav-tabs">
                ...
                <li>
                    <a href="#panel-2" data-toggle="tab" style="Background-Color: #FE9A2E; color: #fff;">失  败 ({})</a>
                </li>
                <li>
                    <a href="#panel-3" data-toggle="tab" style="Background-Color: #FF4000; color: #fff;">错  误 ({})</a>
                </li>
                <li>
    ...
    def genHtmlReport(self, html_template):
        ...
        for data_key in self.report_cases.keys():
            ...
            if 'right_cases' == data_key:
                ...
                str_untreaded_cases = ''.join([str_untreaded_cases,\
                                                '                   <tr class="untreaded">\n',\
                                                export_lable_title,
                                                '                   </tr>\n'])
                for data_case in self.report_cases[data_key][:-1]:
                    str_untreaded_cases = ''.join([str_untreaded_cases, '                   <tr class="untreaded">\n'])
                    for data in data_case:
                        if '详细' in data:
                            str_untreaded_cases = ''.join([str_untreaded_cases,REPORT_Table_Detail.format(data)])
                        else:
                            str_untreaded_cases = ''.join([str_untreaded_cases, REPORT_Table_Data.format(data)])
                    str_untreaded_cases = ''.join([str_untreaded_cases, '                   </tr>\n'])

            elif 'fail_cases' == data_key:
                str_fail_cases = ''.join([str_fail_cases,\
                                                '                   <tr class="fail">\n',\
                                                export_lable_title,
                                                '                   </tr>\n'])
                for data_case in self.report_cases[data_key][:-1]:
                    str_fail_cases = ''.join([str_fail_cases, '                   <tr class="fail">\n'])
                    for data in data_case:
                        if '详细' in data:
                            str_fail_cases = ''.join([str_fail_cases,REPORT_Table_Detail.format(data)])
                        else:
                            str_fail_cases = ''.join([str_fail_cases, REPORT_Table_Data.format(data)])
                    str_fail_cases = ''.join([str_fail_cases, '                   </tr>\n'])
            ...
            '''
        export_lable_datas = REPORT_TMPL_Lable.format(len(self.report_cases['all_cases']),
        ...
        html = template(html_template, report_title=self.html_title, theme=self.pie_theme,
                        start_time=self.test_info[0], end_time=self.test_info[1], used_time=self.test_info[2],
                        sum_all_cases=self.test_info[3], sum_executed_cases=self.test_info[4],
...
'''
    report_cases = {'all_cases': [
        ...
        'error_cases': [['理财4', '2', '产品列表', 'list', '55t', 'Fail', '详细', case_detail],
                        ['理财5', '2', '产品列表', 'list', '55t', 'Fail', '详细', case_detail]],
        'untreaded_cases': [
